...
13a. **Saving a signed document into a Monday File column is automatic-only from this server — there is no on-demand "push this file to that column" tool, and none is needed.** Two workflow settings make the *backend* write the file itself once their trigger fires: Sign anywhere (`fileColumnId`, see 7a) on signing completion, and Generate document (`outputColumnId`) on the configured status column reaching its label. Neither is retroactive — turning one on after a document is already signed does not backfill it. For an already-completed document, or when neither setting is configured, chain `getsign_download_signed_documents` (presigned download URL) into a Monday connector's asset-upload tools (e.g. `get_asset_upload_url` → PUT the bytes → `finalize_asset_upload` with `board_id`/`item_id`/`column_id`) — confirmed against `getsign-backend` that no standalone REST endpoint exists for an arbitrary on-demand file-to-column write either, so this cross-server chain is the only path, not a gap this repo should fill.
